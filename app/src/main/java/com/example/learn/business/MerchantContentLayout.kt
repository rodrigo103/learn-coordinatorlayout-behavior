...
class MerchantContentLayout(context: Context, attrs: AttributeSet?) : ConstraintLayout(context, attrs) {
    ...
    private var binding: MerchantContentLayoutBinding =
        MerchantContentLayoutBinding.inflate(LayoutInflater.from(context), this, true)
    ...
    override fun onWindowFocusChanged(hasWindowFocus: Boolean) {
        ...
        if (!firstLayout) {
            ...
            binding.vAvatar.stateSave(R.id.vs1)
            val tx: Float = ((width - binding.vAvatar.width) / 2 - (binding.vAvatar.layoutParams as MarginLayoutParams).leftMargin).toFloat()
            binding.vAvatar.stateSave(R.id.vs2).tx(tx).ty(dp(10).toFloat())
        }
    }
